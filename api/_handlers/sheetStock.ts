// sheetStock.ts
// Backend for Google Sheets SCHWAB_STOCK() — underlying stock analytics per request.
//
// Apps Script calls:
//   GET https://therpghub.vercel.app/api/schwab?action=sheetStock&symbol=<TICKER>&field=<name>[&key=<SHEET_KEY>]
//
// Contract (must stay stable for Sheets):
//   - Success: { value: number | null }
//   - Failure: { error: string }  → Apps Script surfaces as "ERR: …"
//
// Fields: beta | rv30 | rv90 | iv30 | iv90 | ivrv30 | ivrv90
...
export async function handler(req: any, res: any): Promise<void> {
  try {
    ...
    if (!fieldRaw || !VALID_FIELDS.includes(fieldRaw as typeof VALID_FIELDS[number])) {
